"""Traduccion offline EN->ES via Marian (proceso aislado)."""
...
_WORKER_TIMEOUT_FROZEN_S = 240.0
...
def _worker_cmd() -> list[str]:
    ...
    return [sys.executable, "-m", "app.offline_worker"]
...
def _translate_once(text: str, source: str, target: str) -> str:
    ...
    if not raw:
        ...
        raise RuntimeError(err or "Offline MT no arranco.")
    data = json.loads(raw)
    if not data.get("ok"):
        raise RuntimeError(data.get("error") or "Offline MT fallo.")
